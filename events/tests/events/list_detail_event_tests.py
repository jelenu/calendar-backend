...
class ListDetailEventTests(APITestCase):
    def setUp(self):
    ...
    def test_list_events_authenticated(self):
        ...
        self.assertEqual(str(response.data[0]['category']), str(self.category.id))
    
    def test_list_events_unauthenticated(self):
        response = self.client.get(reverse('event-list'), format='json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_list_events_not_owner(self):
    ...
    def test_detail_event_authenticated(self):
        access_token = self.get_token_for_user(self.user)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {access_token}')
        ...
        self.assertEqual(str(response.data['category']), str(self.category.id))

    def test_detail_event_unauthenticated(self):
        response = self.client.get(self.url, format='json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_detail_event_not_owner(self):
        access_token = self.get_token_for_user(self.user2)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {access_token}')
        response = self.client.get(self.url, format='json')
